...
class VectorGeoreferencerDialog(QWidget):
    def __init__(self, iface, vb, parent = None):
        super(VectorGeoreferencerDialog, self).__init__(parent)
    ...
    def checkRequirements(self):
        ...
        if tbl is None:
          ...
          return
        if pl is None:
          self.displayMsg( "You must select a vector (line) layer which defines the control points pairs !", True )
          return

        if len(tbl.selectedFeaturesIds())>0:
          self.restrictBox_toGeorefLayer.setEnabled(True)
        else:
          self.restrictBox_toGeorefLayer.setEnabled(True)
          self.restrictBox_toGeorefLayer.setChecked(False)
          self.restrictBox_toGeorefLayer.setEnabled(False)
          
        if len(pl.selectedFeaturesIds())>0:
          self.restrictBox_pairsLayer.setEnabled(True)
        else:
          self.restrictBox_pairsLayer.setEnabled(True)
          self.restrictBox_pairsLayer.setChecked(False)
          self.restrictBox_pairsLayer.setEnabled(False)
