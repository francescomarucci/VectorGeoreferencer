...
class AffineTransformer(Transformer):
    def __init__(self, pairsLayer ):
        ...
        for i in range(cc00):
          ...
          to_x=Decimal(self.pointsA[i].x())
          to_y=Decimal(self.pointsA[i].y())
          from_x=Decimal(self.pointsB[i].x())
          from_y=Decimal(self.pointsB[i].y())
        ...
        self.a=(bb0*inv_cc01+bb1*inv_cc11+bb2*inv_cc12)
        self.b=(bb0*inv_cc02+bb1*inv_cc12+bb2*inv_cc22)
        self.c=(aa0*inv_cc01+aa1*inv_cc11+aa2*inv_cc12)
        self.d=(aa0*inv_cc02+aa1*inv_cc12+aa2*inv_cc22)
        self.tx=(bb0*inv_cc00+bb1*inv_cc01+bb2*inv_cc02)
        self.ty=(aa0*inv_cc00+aa1*inv_cc01+aa2*inv_cc02)
        
        #QgsMessageLog.logMessage("a: "+str(self.a)+", b: "+str(self.b)+", c: "+str(self.c)+", d: "+str(self.d)+", tx: "+str(self.tx)+", ty:"+str(self.ty), 'AffineTransformer Parameters')

        
    def map(self, p):
      
        return QgsPoint(self.a*Decimal(p[0])+self.b*Decimal(p[1])+self.tx, self.c*Decimal(p[0])+self.d*Decimal(p[1])+self.ty)
